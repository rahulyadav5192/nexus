...
@section('title', 'Operations — Nexus Group Holdings')
@section('meta_description', 'The Group\'s operations across the gold value chain — sourcing, refining, manufacturing, wholesale, retail and investment.')
@section('active_page', 'operations')
...
<section id="operations">
  <div class="wrap">
    ...
    <div class="op-grid" id="opGrid">
      ...
      <article class="op reveal{{ $item->reveal_delay ? ' ' . $item->reveal_delay : '' }}" data-cat="{{ $item->category_slug }}">
        <div class="op-media">
          <span class="cat-tag">{{ $item->category_tag }}</span>
          <img loading="lazy" src="{{ $item->image_url }}" alt="{{ $item->image_alt ?? $item->name }}">
        </div>
        <div class="op-body">
          <h3>{{ $item->name }}</h3>
          ...
          <div class="op-hl">
            ...
            <span class="h">
              <span class="hn">{{ $highlight['value'] ?? '' }}@if(!empty($highlight['suffix']))<span style="font-size:.5em">{{ $highlight['suffix'] }}</span>@endif</span>
              <span class="hl-l">{{ $highlight['label'] ?? '' }}</span>
            </span>
            @endforeach
          </div>
          @elseif(!empty($item->bullet_points))
          <ul class="op-list">
...
<section class="cta-video-band">
  ...
  <div class="wrap cta-wrap">
    ...
    <div class="reveal d3" style="display:flex;gap:1rem;flex-wrap:wrap;justify-content:center;margin-top:2.6rem">
      <a class="btn solid reveal d2" href="{{ route('leadership') }}"><span>Meet The Leadership</span> <span class="arr">&rarr;</span></a>
    </div>
  </div>
</section>
