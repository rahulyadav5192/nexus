...
(function () {
  ...
  if (form) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      if (firstInvalid) {
        msg.style.color = 'var(--bronze)'; msg.textContent = 'Please complete your name, email and message.'; 
        firstInvalid.focus();
        return;
      }
      var body = 'Name: ' + form.name.value.trim() + '%0D%0A' + 'Company: ' + form.company.value.trim() + '%0D%0A'
        + 'Email: ' + form.email.value.trim() + '%0D%0A' + 'Phone: ' + form.phone.value.trim() + '%0D%0A%0D%0A'
        + encodeURIComponent(form.message.value.trim());
      window.location.href = 'mailto:info@nexusgroupholdings.com?subject=' + encodeURIComponent(form.subject.value || ('Business enquiry — ' + form.name.value.trim())) + '&body=' + body;
      msg.style.color = 'var(--gold)'; msg.textContent = 'Opening your email client… if nothing happens, write to info@nexusgroupholdings.com'; form.reset();
    });
  }
