...
@section('section_name', 'Blog Sections List')
                ...
                <div class="row">
                    <div class="col-lg-12 margin-tb">
                        ...
                        <div class="pull-right">
                            <a class="btn btn-success" href="{{ route('blogs.sections.create',$blog->id) }}"> Add New Section</a>
                        </div>
                    </div>
                </div>
                @endcan
                ...
                <table id="small-table" class="table table-bordered">
                  ...
                  <tbody id="tablecontents">
                    ...
                    <tr class="row1" data-id="{{ $values->id }}">
                      ...
                      <td>
                        @can('blogs-edit')
                            <a href="{{ route('blogs.sections.edit',[$blog->id,$values->id]) }}" title="Edit" style="margin-right: 10px; display: inline-block;">
                              <i class="fa fa-wrench"></i>
                            </a>
                        @endcan
                        @can('blogs-delete')
                            {!! Form::open(['method' => 'DELETE','route' => ['blogs.sections.delete', [$blog->id,$values->id]],'style'=>'display:inline','onclick'=>'return confirm("Are you sure?")']) !!}
  ...
  $(function () {
    ...
    function sendOrderToServer() {
      ...
      $.ajax({
        type: "POST", 
        dataType: "json", 
        url: "<?= route('blogsSectionsOrderUpdate',$blog->id)?>",
        data: {
          order:order,
          _token: '{{csrf_token()}}'
        },
