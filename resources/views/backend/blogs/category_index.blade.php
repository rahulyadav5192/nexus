...
@section('section_name', 'Categories List')
                ...
                <div class="row">
                    <div class="col-lg-12 margin-tb">
                        ...
                        <div class="pull-right">
                            <a class="btn btn-success" href="{{ route('blog_cat.create') }}"> Add New</a>
                        </div>
                    </div>
                </div>
                @endcan
                    ...
                    <tr class="row1" data-id="{{ $values->id }}">
                      ...
                      <td>
                        @can('blog-category-edit')
                            <a href="{{ route('blog_cat.edit',$values->id) }}" title="Edit" style="margin-right: 10px; display: inline-block;">
                              <i class="fa fa-wrench"></i>
                            </a>
                        @endcan
                        @can('blog-category-delete')
                            {!! Form::open(['method' => 'DELETE','route' => ['blog_cat.destroy', $values->id],'style'=>'display:inline','onclick'=>'return confirm("Are you sure?")']) !!}
  ...
  $(function () {
    ...
    function sendOrderToServer() {
      ...
      $.ajax({
        type: "POST", 
        dataType: "json", 
        url: "<?= route('blogCategoriesOrderUpdate')?>",
        data: {
          order:order,
          _token: '{{csrf_token()}}'
        },
