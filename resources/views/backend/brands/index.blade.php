...
@section('section_name', 'Brands List')
              ...
              <table id="small-table" class="table table-bordered">
                ...
                <tbody id="tablecontents">
                  ...
                  <tr class="row1" data-id="{{ $values->id }}">
                    ...
                    <td>{{ $values->name }}</td>
                    <td><img src="{{asset('uploads/brands/'.$values->image)}}" width="200px" height="100px"></td>
                    <td>{{ ($values->status == 1)? "Active" : "Not Active" }}</td>
                    <td>
                      @can('brands-edit')
                      <a href="{{ route('brands.edit',$values->id) }}" title="Edit" style="margin-right: 10px; display: inline-block;">
                        <i class="fa fa-wrench"></i>
                      </a>
                      @endcan
                      @can('brands-delete')
                      {!! Form::open(['method' => 'DELETE','route' => ['brands.destroy', $values->id],'style'=>'display:inline','onclick'=>'return confirm("Are you sure?")']) !!}
