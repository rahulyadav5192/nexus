...
@section('section_name', 'Service Categories List')
              ...
              <table id="small-table" class="table table-bordered">
                ...
                <tbody id="tablecontents">
                  ...
                  <tr class="row1" data-id="{{ $values->id }}">
                    ...
                    <td>
                      @can('categories-edit')
                      <a href="{{ route('categories.edit',$values->id) }}" title="Edit" style="margin-right: 10px; display: inline-block;">
                        <i class="fa fa-wrench"></i>
                      </a>
                      @endcan
                      @can('categories-delete')
                      {!! Form::open(['method' => 'DELETE','route' => ['categories.destroy', $values->id],'style'=>'display:inline','onclick'=>'return confirm("Are you sure?")']) !!}
  ...
  $(function() {
    ...
    function sendOrderToServer() {
      ...
      $.ajax({
        type: "POST",
        dataType: "json",
        url: "<?= route('categoriesOrderUpdate') ?>",
        data: {
          order: order,
          _token: '{{csrf_token()}}'
        },
