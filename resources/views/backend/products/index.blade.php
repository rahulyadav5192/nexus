...
@section('section_name', 'Sub Categories List')
                ...
                <table id="small-table" class="table table-bordered">
                  <thead>
                    <tr>
                      ...
                      <th>Category Name</th>
                      <th>Product Name</th>
                      <th>Thumbnail Image (270 x 270 px)</th>
                      <th>Status</th>
                      <th>Actions</th>
                    </tr>
                  </thead>
                  <tbody id="tablecontents">
                    ...
                    <tr class="row1" data-id="{{ $values->id }}">
                      ...
                      <td>{{ $values->name }}</td>
                      <td><img src="{{asset('uploads/products/'.$values->image)}}" width="100px" height="100px"></td>
                      <td>{{ ($values->status == 1)? "Active" : "Not Active" }}</td>
                      <td>
                        @can('products-edit')
                            <a href="{{ route('products.edit',$values->id) }}" title="Edit"><i class="fa fa-wrench"></i></a>
                        @endcan
                        @can('products-delete')
                            {!! Form::open(['method' => 'DELETE','route' => ['products.destroy', $values->id],'style'=>'display:inline','onclick'=>'return confirm("Are you sure?")']) !!}
