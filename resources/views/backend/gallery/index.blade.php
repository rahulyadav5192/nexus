...
@section('page_name', 'Gallery Management')
@section('section_name', 'Gallery List')
              ...
              <table id="small-table" class="table table-bordered">
                ...
                <tbody id="tablecontents">
                  ...
                  <tr class="row1" data-id="{{ $values->id }}">
                    ...
                    <td>{{ $values->name }}</td>
                    <td><img src="{{asset('uploads/gallery/'.$values->image)}}" width="200px" height="100px"></td>
                    <td>{{ ($values->status == 1)? "Active" : "Not Active" }}</td>
                    <td>
                      @can('gallery-edit')
                      <a href="{{ route('gallery.edit',$values->id) }}" title="Edit" style="margin-right: 10px; display: inline-block;">
                        <i class="fa fa-wrench"></i>
                      </a>
                      @endcan
                      @can('gallery-delete')
                      {!! Form::open(['method' => 'DELETE','route' => ['gallery.destroy', $values->id],'style'=>'display:inline','onclick'=>'return confirm("Are you sure?")']) !!}
