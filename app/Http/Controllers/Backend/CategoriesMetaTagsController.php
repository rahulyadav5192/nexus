...
class CategoriesMetaTagsController extends Controller
{
    /**
    ...
    {
        return view('backend.meta_tags.category_index');
    }
    ...
    public function showTable(Request $request){
         ...
         // Total records
         $totalRecords = MetaTags::select('count(*) as allcount')->where('category_id','!=',NULL)->count();
         ...
         foreach($records as $record){
            $actions    = " ";
            $actions   .= "<a title='Edit Details' href='".route('meta-tags.category.edit',$record->id)."'  class='pull-left' style='margin-right: 3px;'><i class='fas fa-edit'></i></a> ";
    ...
    public function create(){
        $keywords = MetaTags::where('category_id','!=',NULL)->get();
        $data = [];
        foreach ($keywords as $page_tag) {
            $data[] = $page_tag->category_id;
        }
        $categories = Categories::whereNotIn('categories.id', $data)
                    ->pluck('categories.name', 'categories.id');

        return view('backend.meta_tags.category_create',compact('categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $admin_activity = AdminActivities::create([
            ...
            'activity' => 'added new keyword for category',
        ]);
    
        return redirect()->route('meta-tags.category')
                        ->with('success','Category keyword added successfully');
    }
    ...
    public function edit($id)
    {
        ...
        return view('backend.meta_tags.category_edit',compact('data'));
    }
